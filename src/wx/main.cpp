...
constexpr const char* kDlgWildcard =
    "DLG conversation files (*.dlg)|*.dlg|All files (*.*)|*.*";
...
const char* tableWildcardForFormat(neotabular::Format format) {
    ...
    default: throw std::invalid_argument("NeoDLG supports semantic XML and JSON table import/export only.");
    }
}
...
public:
    AddFieldDialog(wxWindow* parent, const std::string& initialParent)
        : wxDialog(parent, wxID_ANY, "Add DLG Field", wxDefaultPosition, wxDefaultSize,
...
public:
    NeoDLGFrame()
        : wxFrame(nullptr, wxID_ANY, "NeoDLG v1.0.0 (DLG editor)", wxDefaultPosition, wxDefaultSize) {
...
private:
...
#endif
        wxIcon fallbackIcon(neodlg_icon_xpm);
    ...
    void buildMenus() {
        ...
        file->Append(ID_New, "&New DLG...");
        file->Append(ID_Open, "&Open DLG...");
        ...
        Bind(wxEVT_MENU, &NeoDLGFrame::onNew, this, ID_New);
        Bind(wxEVT_MENU, &NeoDLGFrame::onOpen, this, ID_Open);
        Bind(wxEVT_MENU, &NeoDLGFrame::onOpenRecent, this, kRecentFileBaseId, kRecentFileBaseId + neosettings::kMaxRecentFiles - 1);
        Bind(wxEVT_MENU, &NeoDLGFrame::onClearRecentFiles, this, kClearRecentFilesId);
        ...
        Bind(wxEVT_MENU, &NeoDLGFrame::onAddField, this, ID_AddField);
        Bind(wxEVT_MENU, &NeoDLGFrame::onDeleteField, this, ID_DeleteField);
        Bind(wxEVT_MENU, &NeoDLGFrame::onCopyCells, this, ID_CopyCells);
        Bind(wxEVT_MENU, &NeoDLGFrame::onPasteCells, this, ID_PasteCells);
        Bind(wxEVT_MENU, &NeoDLGFrame::onFilterPrompt, this, ID_Filter);
        Bind(wxEVT_MENU, &NeoDLGFrame::onClearFilter, this, ID_ClearFilter);
        Bind(wxEVT_MENU, &NeoDLGFrame::onFilterSelectedColumn, this, ID_FilterColumn);
        Bind(wxEVT_MENU, &NeoDLGFrame::onClearSelectedColumnFilter, this, ID_ClearColumnFilter);
        Bind(wxEVT_MENU, &NeoDLGFrame::onClearAllFilters, this, ID_ClearAllFilters);
        Bind(wxEVT_MENU, &NeoDLGFrame::onMoveColumnLeft, this, ID_MoveColumnLeft);
        Bind(wxEVT_MENU, &NeoDLGFrame::onMoveColumnRight, this, ID_MoveColumnRight);
        Bind(wxEVT_MENU, &NeoDLGFrame::onResetColumnOrder, this, ID_ResetColumnOrder);
        Bind(wxEVT_MENU, &NeoDLGFrame::onResetRowOrder, this, ID_ResetRowOrder);
        ...
        Bind(wxEVT_MENU, &NeoDLGFrame::onExpandTree, this, ID_ExpandTree);
        Bind(wxEVT_MENU, &NeoDLGFrame::onCollapseTree, this, ID_CollapseTree);
        ...
        Bind(wxEVT_BUTTON, &NeoDLGFrame::onAddField, this, ID_AddField);
        Bind(wxEVT_BUTTON, &NeoDLGFrame::onDeleteField, this, ID_DeleteField);
        Bind(wxEVT_BUTTON, &NeoDLGFrame::onClearFilter, this, ID_ClearFilter);
    }
    ...
    void buildWindow() {
        ...
        auto* fileBox = new wxStaticBoxSizer(wxVERTICAL, panel, "DLG");
        ...
        if (filterText_) filterText_->Bind(wxEVT_TEXT, &NeoDLGFrame::onFilterText, this);
        Bind(wxEVT_GRID_CELL_CHANGED, &NeoDLGFrame::onCellChanged, this, ID_Grid);
        Bind(wxEVT_GRID_LABEL_RIGHT_CLICK, &NeoDLGFrame::onGridLabelRightClick, this, ID_Grid);
        Bind(wxEVT_TREE_SEL_CHANGED, &NeoDLGFrame::onTreeSelectionChanged, this, ID_ElementTree);
        Bind(wxEVT_TREE_ITEM_ACTIVATED, &NeoDLGFrame::onTreeActivated, this, ID_ElementTree);
    ...
    bool maybeSave() {
        ...
        const int result = wxMessageBox("The current DLG has unsaved changes. Save it?", "Unsaved Changes",
    ...
    void chooseAndOpenDlg(const std::filesystem::path& initialDirectory = {}) {
        auto file = wxui::chooseOpenFile(this, "Open DLG", kDlgWildcard, initialDirectory);
        if (!file) return;
        try {
        ...
        try {
            ...
            if (format == neotabular::Format::Xml || format == neotabular::Format::Json) {
                if (neoview::hasAnyFilter(viewState())) {
                    throw std::invalid_argument("Semantic DLG XML/JSON export preserves hierarchy and does not support row filtering.");
                }
    ...
    void onExportPatcherPackage(wxCommandEvent&) {
        ...
        try {
            ...
            std::string defaultPatchName = model().filename().empty()
                ? std::string("modified.dlg")
                : model().filename().filename().string();
            if (defaultPatchName.empty()) defaultPatchName = "modified.dlg";
            ...
            GffModel originalModel;
            originalModel.load(*originalPath);
            auto project = neodlg::patcher::diffDlgPatcher(originalModel.gff(), model().gff(), *patchName, true, *originalPath);
            neotsl::throwIfUnsupported(project);
            neotsl::writePackage(project, *outputDir, true);

            wxui::showMessage(this,
                              "TSL/HoloPatcher Package",
                              "Wrote changes.ini and staged the clean baseline DLG in:\n" +
                                  pathText(*outputDir) +
                                  "\n\nNew EntryList/ReplyList nodes use dynamic ListIndex tokens; added dialogue links are rewired with 2DAMEMORY field-path tokens.");
        } catch (const std::exception& ex) {
            wxui::showError(this, ex);
        }
    }
    ...
    bool save(bool saveAs) {
        ...
        try {
            ...
            if (saveAs || target.empty()) {
                const std::string defaultName = target.empty() ? std::string("new.dlg") : target.filename().string();
                auto chosen = wxui::chooseSaveFile(this, "Save DLG", kDlgWildcard, defaultName);
    ...
    void onTreeActivated(wxTreeEvent& event) {
        ...
        auto value = wxui::promptText(this, "Edit DLG Value", row.label + " (" + row.type + "):", row.value);
        if (!value) return;
        try {
    ...
    void refreshTree() {
        ...
        const std::string rootText = model().loaded()
            ? (pathText(model().filename()).empty() ? std::string("New DLG File") : pathText(model().filename()))
    ...
    void refreshAll() {
        ...
        SetTitle(wxui::toWx("NeoDLG v1.0.0" + std::string(model().dirty() ? " *" : "") + " (DLG editor)"));
        ...
        applyDarkMode();
        if (GetStatusBar()) {
            wxui::setStatusText(*this, wxui::toWx(model().loaded() ? pathText(model().filename()) : std::string("No DLG loaded")), 0);
