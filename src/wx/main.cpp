...
public:
    NeoDLGFrame():wxFrame(nullptr,wxID_ANY,wxui::toWx(std::string("NeoDLG v")+neodlg::kVersion)) {
        wxIconBundle icons;
#if defined(__WXMSW__)
        wxIcon native("neodlg",wxBITMAP_TYPE_ICO_RESOURCE);if(native.IsOk())icons.AddIcon(native);
#endif
        wxIcon fallback(neodlg_icon_xpm);if(fallback.IsOk())icons.AddIcon(fallback);
        SetIcons(icons);
        neomodules::Context context;
        context.titleChanged=[this](const wxString& title){SetTitle(title);};
        context.closeRequested=[this]{Close();};
        panel_=neodlg::ui::createEditorPanel(this,std::move(context));
    ...
    ~NeoDLGFrame() override {DestroyChildren();}
...
private:
    neodlg::ui::DLGEditorPanel* panel_{};
    neosettings::AppSettings settings_{"NeoDLG"};
};
class NeoDLGApp final:public wxApp {
public:bool OnInit()override {
    SetAppName("NeoDLG");SetVendorName("Neo Tools");wxInitAllImageHandlers();
    auto* frame=new NeoDLGFrame;frame->Show();
